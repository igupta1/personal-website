...
def extract_job_role(evidence: str) -> str:
    """Extract job role from evidence string like 'Indeed posting for Marketing Manager (last 14 days)'"""
    import re
    if not evidence:
        return ''
    
    # Try to match "Indeed posting for [Job Role]"
    match = re.search(r'Indeed posting for\s+(.+?)\s*\(', evidence)
    if match:
        return match.group(1).strip()
    
    # Fallback: try to extract anything after "for" 
    match = re.search(r'posting for\s+(.+?)(?:\s*\(|$)', evidence, re.IGNORECASE)
    if match:
        return match.group(1).strip()
...
def read_leads_from_csv(filepath: str) -> list:
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            # Get evidence and extract job role from it
            evidence = row.get('Evidence They Need Marketing Help', '') or row.get('Evidence', '')
            job_role = extract_job_role(evidence)
            
            # Get job link - try multiple possible column names
            job_link = row.get('Source Link(s)', '') or row.get('Job Link', '') or row.get('Source Links', '')
            ...
            lead = {
                ...
                'location': row.get('City / Neighborhood', '') or row.get('City/Neighborhood', ''),
