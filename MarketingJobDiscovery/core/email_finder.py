...
class ApolloEmailFinder:
    ...
    async def find_emails(
        ...
        Args:
            ...
            company_results: Company result dicts from the orchestrator,
                             used to get domain for each company.

        Returns:
            List of EmailLookupResult, one per input decision maker.
        """
        # Build domain lookup from company results
        domain_by_company = {
            r["company"]: r.get("domain", "")
        ...
        for dm in decision_makers:
            ...
            lookup_items.append({
                ...
                "first_name": first,
                "last_name": last,
                "domain": domain_by_company.get(dm.company_name, ""),
            })
