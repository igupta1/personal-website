"""
AgencySalesDiscovery CLI — Find Marketing Agencies hiring for sales/marketing roles.

Usage:
    python -m AgencySalesDiscovery run [--dry-run] [--max-searches N] [-v]
    python -m AgencySalesDiscovery status
"""
...
def write_csv(output_dir: Path, rows: List[Dict]) -> Path:
    ...
    csv_path = output_dir / "agency_sales_leads.csv"
...
async def run_pipeline(config: Config, dry_run: bool = False, max_searches: int = None):
    """Execute the full agency sales discovery pipeline."""
    start_time = datetime.now()
    max_s = max_searches or config.max_searches_per_run

    # Step 1: SerpAPI search
    print("\n" + "=" * 70)
    print("Step 1: Searching for sales/marketing roles at Marketing Agencies...")
    print("=" * 70)
    ...
    if config.gemini_api_key:
        ...
        # Filter and merge
        rejected_not_agency = 0
        rejected_too_large = 0
        for listing in new_listings:
            norm = listing.company_name.lower().strip()
            dm = dm_by_company.get(norm)

            if dm and not dm.is_verified_agency:
                rejected_not_agency += 1
                print(f"  REJECTED (not agency): {listing.company_name} — {dm.not_found_reason or 'N/A'}")
                seen.add(norm)
                continue
        ...
        print(f"  Rejected (not agency): {rejected_not_agency}")
...
def cmd_status(args):
    ...
    print("=" * 70)
    print("Agency Sales Discovery — Status")
    print("=" * 70)
    ...
    # Output file
    csv_path = config.output_dir / "agency_sales_leads.csv"
    if csv_path.exists():
...
def main():
    parser = argparse.ArgumentParser(
        prog="python -m AgencySalesDiscovery",
        description="Find Marketing Agencies hiring for sales/marketing roles",
    )
    subparsers = parser.add_subparsers(dest="command", help="Commands")

    # Run command
    run_parser = subparsers.add_parser("run", help="Run agency sales discovery pipeline")
