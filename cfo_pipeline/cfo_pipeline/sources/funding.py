"""Funding RSS source — ported from insurance_pipeline.sources.funding.

Emits ``FUNDING_RAISED`` for the CFO pipeline. A fresh Series A/B/C
round at a US SMB is a real fractional-CFO trigger: the company just
took outside capital, has new reporting obligations to a board, and
typically can't yet justify a $300K full-time CFO comp package. That's
the buying window.

Independent from insurance_pipeline.sources.funding. Mirror code,
mirror behavior; no cross-imports between pipelines.
"""
...
def fetch(*, since: datetime, limit: int | None = None) -> list[LeadCandidate]:
    candidates: list[LeadCandidate] = []
    for feed_url in (_TECHCRUNCH_FEED, _PRNEWSWIRE_FEED):
        try:
            candidates.extend(_fetch_from_rss(feed_url, since))
        except Exception:
            _log.exception("funding: fetcher %s failed entirely", feed_url)
